<template>
  <div id="timer">
     ...
     <h2 v-if="otmode">Running in OT Pen Testing: {{ parseInt(elapsedTime/36000) }}:{{ parseInt(elapsedTime/600)%60 }}:{{ parseInt(elapsedTime/10)%60 }}:{{ elapsedTime%10 }}</h2>
...
export default {
  ...
  methods: {
    ...
    itstart: function () {
      ...
      this.$axios.get('http://google.co.jp',{params:{data:this.message}})
        .then(function(response){
          console.log(response.data.message)  //Logging the calculation results returned from the backend in console.log
    ...
    otstart: function () {
      ...
      let self = this;
      this.state="getting data"
      this.$axios.get('http://yahoo.co.jp',{params:{data:this.message}})
        .then(function(response){
          console.log(response.data.message)  //Logging the calculation results returned from the backend in console.log
          this.result= response.data.message
          this.state="done"    
